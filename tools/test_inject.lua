...
do
  ...
  assert_true(source:find('require("themeengine")', 1, true),
    "boot wires in the theme engine")
  assert_true(source:find('require("themepreload")', 1, true),
    "boot wires in the theme preloader")
  ...
  assert_true(source:find('titles = { ["Steam"] = true }, urls = { "browserType=4" }', 1, true)
      and source:find("build_menu_assets(true, theme_key)", 1, true),
    "the menu bundle channel matches the desktop shell (Steam title / browserType=4)")
...
-- select_targets(targetsList, wantedTitles, wantedOrigins, wantedUrlFragments)
-- is the pure matcher the injector uses to decide which CEF targets receive
-- the assets.
...
do
  ...
  local source = f:read("*a")
  f:close()
  -- The anonymous/parental page fallback is owned by the page connection via
  -- Fetch.requestPaused. (Browser-wide Target.setAutoAttach now also exists, but
  -- only for the opt-in custom-theme browser gateway; the login observer and the
  -- parental path never use it -- see the gateway tests above.)
